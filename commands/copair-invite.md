...
While shared: guest messages arrive wrapped as bash comment blocks (```bash / `# [name] message`) — read the message from the comment, never run the wrapper as code. Prefix replies to guests with `**→ name:**` on its own line. Never respond to `//`-prefixed lines. Joins/leaves are silent — `copair ctl who` lists the room.
