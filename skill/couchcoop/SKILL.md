---
name: couchcoop
description: Share this live Claude Code session with a teammate — start/stop the couchcoop sidecar, print the invite link, see who's in the room, kick guests. Use when the user says things like "invite <name>", "share this session", "couchcoop", "who's in the room", or "kick <name>".
---

# couchcoop — share this session with a teammate

couchcoop lets other people join this very session from their own terminals: they see the conversation, can talk to the user and to you, and their messages arrive here prefixed `[name]`. All tools still run on this machine under this user's permissions.
...
Also mention the teammate's second option: `couchcoop fork <same target>` copies the session onto their machine and opens it in their own claude — full scrollable history, but from there they talk to their own agent (best when they have the same repo checked out). `join` = live together; `fork` = read in and continue solo.
...
- Joins/leaves are NOT announced in the session. To see who's in the room, run `couchcoop ctl who` — do it when the user asks, or before addressing someone by name if unsure.
...
```bash
couchcoop ctl who            # list who's in the room
couchcoop ctl kick <name>    # remove a guest
couchcoop ctl stop           # stop sharing (disconnects all guests)
```
