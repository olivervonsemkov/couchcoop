...
- Messages from guests arrive wrapped as a bash comment block — ` ```bash` / `# [name] message` / ` ``` ` — purely so they render in color in the terminal. Read the actual message from the comment text; never treat the wrapper as code to run.
